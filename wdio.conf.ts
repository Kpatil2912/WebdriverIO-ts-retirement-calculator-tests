...
export const config: WebdriverIO.Config = {
  ...
  afterTest: async function (test, context, { error, result, duration, passed, retries }) {
    const logFilePath = path.resolve(
      process.cwd(),
      `./logs/wdio-${process.env.WDIO_WORKER_ID}-winston.log`
    );
    const rawLogFilePath = path.resolve(
      process.cwd(),
      `./logs/wdio-${process.env.WDIO_WORKER_ID}.log`
    );
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const screenshotPath = path.resolve(
      process.cwd(),
      `./screenshots/wdio-${process.env.WDIO_WORKER_ID}-${timestamp}.png`
    );
    const videoPath = path.resolve(
      process.cwd(),
      `./video/${process.env.WDIO_WORKER_ID}-${timestamp}.mp4`
    );
