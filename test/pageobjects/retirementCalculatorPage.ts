...
export default class RetirementCalculatorPage extends Page {
  ...
  public async fillDefaultValues(
    retirementCalculatorInputDao: RetirementCalculatorInputDao
  ): Promise<this> {
    ...
    await this.setElementValue(
      ...
      retirementCalculatorInputDao.getRetirementIncomeDependantAge()
    );
    await this.setElementValue(
      this.retirementDuration(),
      retirementCalculatorInputDao.getRetirementIncomeDependantAge()
    );
    await this.clickWhenVisible(
      this.inflationInclusionRadio(retirementCalculatorInputDao.getInflationLabel())
    );
    await this.setElementValue(
      this.expectedInflationRate(),
      retirementCalculatorInputDao.getExpectedInflationRate()
    );
    await this.setElementValue(
      this.retirementAnnualIncome(),
      retirementCalculatorInputDao.getRetirementAnnualIncome()
    );
    await this.setElementValue(
      this.preRetirementRoi(),
      retirementCalculatorInputDao.getPreReturn()
    );
    await this.setElementValue(
      this.postRetirementRoi(),
      retirementCalculatorInputDao.getPostReturn()
    );
  ...
  public async setEmptyCurrentAge(
    retirementCalculatorInputDao: RetirementCalculatorInputDao
  ): Promise<this> {
    await this.setElementValue(this.currentAge(), retirementCalculatorInputDao.getEmptyValue());
    return this;
  }
  public async setLessRetirementAge(
    retirementCalculatorInputDao: RetirementCalculatorInputDao
  ): Promise<this> {
    await this.setElementValue(
      this.retirementAge(),
      retirementCalculatorInputDao.getLessRetirementAge()
    );
    await this.setElementValue(this.currentAge(), retirementCalculatorInputDao.getCurrentAge());
    await this.setElementValue(
      this.currentAnnualIncome(),
      retirementCalculatorInputDao.getCurrentIncome()
    );
    await this.setElementValue(
      this.currentTotalSaving(),
      retirementCalculatorInputDao.getSavings()
    );
    await this.setElementValue(
      this.currentAnnualSaving(),
      retirementCalculatorInputDao.getContribution()
    );
    await this.setElementValue(
      this.savingsIncreaseRate(),
      retirementCalculatorInputDao.getIncreaseRate()
    );
    await this.clickOnSocialBenefitsRadioButton(retirementCalculatorInputDao);
    await this.clickWhenVisible(
      this.maritalStatusRadioButton(retirementCalculatorInputDao.getMaritalStatus())
    );
    return this;
  }
  public async setEqualRetirementAge(
    retirementCalculatorInputDao: RetirementCalculatorInputDao
  ): Promise<this> {
    await this.setElementValue(
      this.retirementAge(),
      retirementCalculatorInputDao.getEqualRetirementAge()
    );
    await this.setElementValue(this.currentAge(), retirementCalculatorInputDao.getCurrentAge());
    await this.setElementValue(
      this.currentAnnualIncome(),
  ...
  public async fillForm(retirementCalculatorInputDao: RetirementCalculatorInputDao): Promise<this> {
    ...
    await this.setElementValue(
      this.retirementAge(),
      retirementCalculatorInputDao.getRetirementAge()
    );
    await this.setElementValue(
      this.currentAnnualIncome(),
      retirementCalculatorInputDao.getCurrentIncome()
    ...
    await this.setElementValue(
      ...
      retirementCalculatorInputDao.getSpouseIncome()
    );
    await this.setElementValue(
      this.currentTotalSaving(),
      retirementCalculatorInputDao.getSavings()
    ...
    await this.setElementValue(
      this.socialSecurityOverride(),
      retirementCalculatorInputDao.getSocialSecurityOverride()
    );
    logger.info('Form filled.');
    allureReporter.endStep();
    return this;
  }
  public async fillFormRequired(
    retirementCalculatorInputDao: RetirementCalculatorInputDao
  ): Promise<this> {
    logger.info('Filling the retirement calculator form with provided input data.');
    allureReporter.startStep('Filling the retirement calculator form with valid data');
  ...
  public async clickOnSocialBenefitsRadioButton(
    retirementCalculatorInputDao: RetirementCalculatorInputDao
  ): Promise<this> {
    await this.clickWhenVisible(
      this.socialBenefitsRadioButton(retirementCalculatorInputDao.getSocialSecurityBenfits())
    );
    return this;
  }
  public async clickOnNoSocialBenefitsRadioButton(
    retirementCalculatorInputDao: RetirementCalculatorInputDao
  ): Promise<this> {
    await this.clickWhenVisible(
      this.socialBenefitsRadioButton(retirementCalculatorInputDao.getNoSocialSecurityBenfits())
    );
    return this;
  }
  ...
  public async acceptCookies(): Promise<void> {
    try {
      await this.acceptCookiesButton().waitForDisplayed({ timeout: 2500 });
